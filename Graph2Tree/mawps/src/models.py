...
class EncoderSeq(nn.Module):
    ...
    def normalize_matrix(self, matrix):
        diag = torch.sqrt(torch.sum(matrix, dim=-1, keepdims=True)+1e-30)
        diag_norm = diag * torch.transpose(diag, -2, -1)
        return matrix / (diag_norm+1e-30)
    ...
    def forward(self, input_seqs, input_lengths, batch_graph, word_exist_mat, ops_embed, temp=0.5, thre=0.5, hidden=None, hard=False):
        ...
        _, pade_outputs = self.gcn(pade_outputs, batch_graph)  # B x S x H
...
class Prediction(nn.Module):
    ...
    def forward(self, node_stacks, left_childs, encoder_outputs, num_pades, padding_hidden, word_word, word_op, word_exist_mat, seq_mask, mask_nums):
        ...
        current_attn = self.attn(current_embeddings.transpose(0, 1), encoder_outputs, seq_mask)  # B x 1 x seq
        ...
        leaf_input = leaf_input.squeeze(1)  # B x N
