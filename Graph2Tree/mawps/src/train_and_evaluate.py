...
def train_tree(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               ...
               merge_optimizer, output_lang, num_pos, batch_graph, temp=0.5, english=False, log_prior=None, log_gt_prior=None, 
               eval_know_inputs=None, know_gt_ww=None, common_dict=None, prior_lam=0.1, gt_prior_lam=0.05):
    ...
    for i in range(batch_size):
        for j in range(input_length[i]):
            j_id = input_batch[i][j]
            if j_id in common_dict:
                for k in np.arange(j+1,input_length[i]):
                    k_id = input_batch[i][k]
                    if k_id in common_dict:
                        know_mask[i][j][k][:] = know_gt_ww[common_dict[j_id],common_dict[k_id]]
                        know_mask[i][k][j][:] = know_gt_ww[common_dict[k_id],common_dict[j_id]]                        
    ...
    # input_var: (max_len, batch_size)
    # encoder_outputs: (max_len, batch_size, dim), problem_output: (batch_size, dim)
    ...
    if log_gt_prior is not None:
        # prior kl divergence
        loss_gt_kl = kl_gt_categorical(ww_prob, log_gt_prior, word_exist_mat, know_mask)
    else:
        loss_gt_kl = 0
    ...
    all_nums_encoder_outputs = get_all_number_encoder_outputs(encoder_outputs, num_pos, batch_size, num_size,
                                                              encoder.hidden_size)
    # all_nums_encoder_outputs: (batch_size, num_size, dim)                                                              
    ...
    for t in range(max_target_length):
        ...
        # num_score: (batch_size, num_size+generate_num_size), op: (batch_size, op_num)
        # num_score: 所有数值(内部+外部)的logit valus, op: 所有运算符的logit valus
        # current_embeddings, current_context: (batch_size, 1, dim)
        # current_nums_embeddings: (batch_size, num_size+generate_num_size, dim)
        ...
        for idx, l, r, node_stack, i, o in zip(range(batch_size), left_child.split(1), right_child.split(1),
            ...
            if i < num_start:
                ...
                try:
                    ...
                    print(i, input_batch[idx], target_batch[idx], t)
                    exit()
    ...
    # logging.info(f"ori_loss: {masked_cross_entropy(all_node_outputs, target, target_length)}, loss_kl: {loss_kl}, loss_wo_kl: {loss_wo_kl}, loss_gt_kl: {loss_gt_kl}")
    loss = masked_cross_entropy(all_node_outputs, target, target_length) + prior_lam*(loss_kl+loss_wo_kl) + gt_prior_lam*loss_gt_kl
    # loss = loss_0 + loss_1
    loss.backward()
    # clip the grad
...
def evaluate_tree(input_batch, input_length, generate_nums, encoder, predict, generate, merge, output_lang, 
                  num_pos, batch_graph, beam_size=5, temp=0.5, thre=0.5, english=False, max_length=MAX_OUTPUT_LENGTH):
    seq_mask = torch.ByteTensor(1, input_length).fill_(0)
    # Turn padded arrays into (batch_size x max_len) tensors, transpose into (max_len x batch_size)
    ...
    for t in range(max_length):
        ...
        while len(beams) > 0:
            ...
            num_score, op, current_embeddings, current_context, current_nums_embeddings = predict(
                b.node_stack, left_childs, encoder_outputs, all_nums_encoder_outputs, padding_hidden, word_word, word_op, word_exist_mat, seq_mask, num_mask)
