...
def main():
	...
	gt_know = np.load("data/svamp_know.npy")
	...
	if config.full_cv:
		...
		for z in range(5):
			...
			if is_train:
				# Select ground_truth knowledge in input_lang.word2index
				common_dict = {}
				common_word_count = 0
				eval_know_inputs = []
			...
			for epoch in range(config.epochs):
				temp = args.temp - (args.temp - 0.1) / (config.epochs-1) * epoch
				prior_lam = 0.1
				gt_prior_lam = 0.1
				loss_total = 0
				...
				for idx in range(len(input_lengths)):
					loss = train_tree(
						...
						embedding_optimizer, encoder_optimizer, predict_optimizer, generate_optimizer, merge_optimizer, input_lang, output_lang, num_pos_batches[idx], 
						graph_batches[idx], temp=temp, log_prior=log_prior, log_gt_prior=log_gt_prior, eval_know_inputs=eval_know_inputs, know_gt_ww=know_gt_ww, common_dict=common_dict, prior_lam=prior_lam, gt_prior_lam=gt_prior_lam)
					loss_total += loss
					...
					with torch.no_grad():
						for train_batch in train_pairs:
							...
							train_res = evaluate_tree(config, train_batch[0], train_batch[1], generate_num_ids, embedding, encoder, predict, generate,
													 merge, input_lang, output_lang, train_batch[4], train_batch[5], batch_graph, test_batch[7], temp=args.temp, thre=args.thre, beam_size=config.beam_size)
				...
				for test_batch in test_pairs:
					...
					test_res = evaluate_tree(config, test_batch[0], test_batch[1], generate_num_ids, embedding, encoder, predict, generate,
											 merge, input_lang, output_lang, test_batch[4], test_batch[5], batch_graph, test_batch[7], temp=args.temp, thre=args.thre, beam_size=config.beam_size)
				...
				# if (epoch+1) % 10 == 0 or epoch ==0:
					# word_word = evaluate_know(encoder, embedding(eval_know_inputs), gt_ww, know_gt_ww, config.temp, device)
				# if epoch == config.epochs-1:
					# torch.save(eval_know_inputs, str(z)+"_eval_know_inputs.pt")
					# torch.save(word_word, str(z)+"_word_word.pt")
					# np.save(str(z)+"_word2index.npy", input_lang.word2index)
		...
		if is_train:
			...
			for epoch in range(config.epochs):
				...
				# if (epoch+1) % 10 == 0 or epoch ==0:
					# word_word = evaluate_know(encoder, embedding(eval_know_inputs), gt_ww, know_gt_ww, config.temp, device)
				# if epoch == config.epochs-1:
					# torch.save(eval_know_inputs, "full_eval_know_inputs.pt")
					# torch.save(word_word, "full_word_word.pt")
					# np.save("full_word2index.npy", input_lang.word2index)
