...
def train_tree_double(encoder_outputs, problem_output, all_nums_encoder_outputs, target, target_length,
                      output_lang, batch_size, padding_hidden, word_word, word_op, word_exist_mat, seq_mask, 
...
#    all_nums_encoder_outputs = all_nums_encoder_outputs.masked_fill_(masked_index.bool(), 0.0)
    ...
    for t in range(max_target_length):
        num_score, op, current_embeddings, current_context, current_nums_embeddings = predict(node_stacks, left_childs, encoder_outputs, all_nums_encoder_outputs, padding_hidden, word_word, word_op, word_exist_mat, seq_mask, num_mask)
...
def train_attn_double(encoder_outputs, decoder_hidden, target, target_length,
                      output_lang, batch_size, word_word, word_op, word_exist_mat, seq_mask, 
    ...
    if random.random() < use_teacher_forcing:
        ...
        for t in range(max_target_length):
            ...
            decoder_output, decoder_hidden = decoder(
                decoder_input, decoder_hidden, encoder_outputs, word_word, word_op, word_exist_mat, seq_mask)
...
#                score = f.log_softmax(decoder_output, dim=1) + rule_mask
                ...
                score1 = score + beam_score
                beam_scores[:, b_idx * decoder.output_size: (b_idx + 1) * decoder.output_size] = score1
            ...
            for k in range(beam_size):
                ...
                temp_beam_pos = (temp_topk / decoder.output_size).long()

                indices = torch.LongTensor(range(batch_size))
                if USE_CUDA:
                    indices = indices.cuda()
                indices1 = indices + temp_beam_pos * batch_size

                temp_hidden = all_hidden.index_select(1, indices1)
                temp_output = all_outputs.index_select(1, indices1)
...
def train_double(input1_batch, input2_batch, input_length, target1_batch, target1_length, target2_batch, target2_length, 
                 ...
                 input_lang, output1_lang, output2_lang, num_pos_batch, num_order_batch, parse_graph_batch, temp0=0.5, log_prior=None, log_gt_prior=None, 
                 eval_know_inputs=None, know_gt_ww=None, common_dict=None, prior_lam=0.1, gt_prior_lam=0.05, beam_size=5, use_teacher_forcing=0.83, english=False):
    ...
    encoder_outputs, encoder_hidden, word_word, ww_prob, word_op, wo_prob = encoder(input1_var, input2_var, input_length, parse_graph_pad, word_exist_mat, predict.ops, temp=temp0, hard=False)
    
    train_index = random.sample(range(len(eval_know_inputs)), int(0.3*len(eval_know_inputs)))
    ...
    encoder_outputs, num_outputs, problem_output = numencoder(encoder_outputs, num_encoder_outputs, num_pos_pad, num_order_pad)
    num_outputs = num_outputs.masked_fill_(masked_index.bool(), 0.0)

    decoder_hidden = encoder_hidden[:decoder.n_layers]  # Use last (forward) hidden state from encoder

    loss_0 = train_tree_double(encoder_outputs, problem_output, num_outputs, target1, target1_length, output1_lang, batch_size, padding_hidden, word_word, word_op, word_exist_mat, seq_mask, num_mask, num_pos_batch, num_order_pad, num_stack1_batch, unk1, encoder, numencoder, predict, generate, merge)
    
    loss_1 = train_attn_double(encoder_outputs, decoder_hidden, target2, target2_length,
                               output2_lang, batch_size, word_word, word_op, word_exist_mat, seq_mask, num_start2, num_stack2_batch, unk2, decoder,beam_size, use_teacher_forcing)
    
    loss = loss_0 + loss_1 + prior_lam*(loss_kl + loss_wo_kl) + gt_prior_lam*loss_gt_kl
    # loss = loss_0 + loss_1
    loss.backward()
...
def evaluate_tree_double(encoder_outputs, problem_output, all_nums_encoder_outputs, 
                         output_lang, batch_size, padding_hidden, word_word, word_op, word_exist_mat, seq_mask, num_mask, 
    ...
    for t in range(max_length):
        ...
        while len(beams) > 0:
            ...
            num_score, op, current_embeddings, current_context, current_nums_embeddings = predict(
                b.node_stack, left_childs, encoder_outputs, all_nums_encoder_outputs, padding_hidden, word_word, word_op, word_exist_mat,
                seq_mask, num_mask)
...
def evaluate_attn_double(encoder_outputs, decoder_hidden, 
                         output_lang, batch_size, word_word, word_op, word_exist_mat, seq_mask, max_length, 
                         decoder, beam_size):
...
def evaluate_double(input1_batch, input2_batch, input_length, generate_num1_ids, generate_num2_ids, 
                    encoder, numencoder, predict, generate, merge, decoder, 
                    input_lang, output1_lang, output2_lang, num_pos_batch, num_order_batch, parse_graph_batch, temp0=0.5, thre=0.5,
    ...
    # Run words through encoder
    word_exist_mat = word_exist(seq_mask)
    encoder_outputs, encoder_hidden, word_word, _ , word_op, _ = encoder(input1_var, input2_var, [input_length], parse_graph_pad, word_exist_mat, predict.ops, temp=temp0, thre=thre, hard=True)
    ...
    tree_beam = evaluate_tree_double(encoder_outputs, problem_output, num_outputs, 
                                     output1_lang, batch_size, padding_hidden, word_word, word_op, word_exist_mat, seq_mask, num_mask, 
                                     max_length, num_pos_batch, num_order_pad, 
                                     encoder, numencoder, predict, generate, merge, beam_size)
    
    attn_beam = evaluate_attn_double(encoder_outputs, decoder_hidden, 
                                     output2_lang, batch_size, word_word, word_op, word_exist_mat, seq_mask, max_length, 
                                     decoder, beam_size)
...
def evaluate_know(encoder, inputs, gt_ww, know_gt_ww, temp, thre):
    ...
    embedded = encoder.embedding1(inputs)  # (word_num, dim)
...
def batch_evaluate_know(encoder, inputs, gt_ww, know_gt_ww, temp, thre):
    ...
    for i in range(num_batch):
        ...
        batch_embed = encoder.embedding1(batch_input) # (batch, dim)
        all_embed = encoder.embedding1(inputs) # (word_num, dim)
    ...
    print("know_pre: ",pre, "know_recall: ",rec)
