...
class SupervisedTrainer(object):
    ...
    def _train_batch(self, input_variables, num_pos, input_lengths, span_length, target_variables, tree, model, batch_size, temp, log_prior, log_gt_prior):
        ...
        train_index = random.sample(range(len(model.eval_know_inputs)), int(0.2*len(model.eval_know_inputs)))
        ww_prob_train, wo_prob_prior = model.encoder.encode_prior(model.eval_know_inputs[train_index].unsqueeze(1), generator_op_embedding)
        know_gt_ww_train = model.know_gt_ww[train_index][:,train_index]
    
        if log_prior is not None:
        # prior kl divergence
            loss_kl = kl_categorical(ww_prob_train, log_prior, know_gt_ww_train.unsqueeze(0).unsqueeze(-1))
            wo_know_mask = torch.zeros((1, know_gt_ww_train.size(0), generator_op_embedding.size(0), 1))
            if self.use_cuda:
        ...
        for epoch_index, epoch in enumerate(range(start_epoch, n_epoch + 1)):
            ...
            with torch.no_grad():
                ...
                # if (epoch_index+1) % 10 == 0 or epoch_index ==0:
                    # word_word = batch_evaluate_know(model.encoder, model.eval_know_inputs, model.gt_ww, model.know_gt_ww, temp0, thre)
                # if epoch == n_epoch:
                    # torch.save(model.eval_know_inputs, "eval_know_inputs.pt")
                    # torch.save(word_word, "word_word.pt")
                    # np.save("word2index.npy", data_loader.vocab_dict)
