...
class DataLoader():
    def __init__(self, args, trim_min_count=0, embed_dim=128, build_wv=False, init_op=False):
        self.args = args
        
        ori_path = "./data/mawps_"
        fold = args.fold
        train_list = read_data_json(ori_path+"train_"+str(fold)+".json")
        # valid_list = read_data_json("./data/valid23k_processed.json")
        test_list = read_data_json(ori_path+"test_"+str(fold)+".json")
        
        data = load_mawps_data("./data/mawps_combine.json")
        test_index = [item['iIndex'] for item in data]

        self.train_list = train_list
        self.test_list = [item for item in test_list if item['id'] in test_index]
        self.wv_path = f"./data/word2vec"+str(fold)+".pt"
        self.op_set = set("+-*/")
        ...
        if init_op:
            op_index = [self.vocab_list.index(i) for i in ['+','*']]
            word_index = [self.vocab_list.index(i) for i in ['more','each']]
            self.embed_vectors[op_index] = self.embed_vectors[word_index]
