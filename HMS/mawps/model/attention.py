...
class HierarchicalAttention(nn.Module):
    ...
    def forward(self, output, span_context, word_contexts, span_mask=None, word_masks=None):
        ...
        for word_context, word_mask in zip(word_contexts, word_masks):
            word_output, word_attn = self.word_attn(output, word_context, word_mask)
            word_outputs.append(word_output.unsqueeze(-2))
            word_attns.append(word_attn.squeeze(1))
        
        goal_word = torch.cat([word_attns[i] * span_attn.squeeze(1)[:,[i]] for i in range(len(word_attns))], dim=1)
        # normal
