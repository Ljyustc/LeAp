...
def kl_gt_categorical(preds, log_gt_prior, word_exist_mat, know_mask, eps=1e-16):
    kl_gt_div = (preds * (torch.log(preds + eps) - log_gt_prior)).masked_fill_((1-know_mask).bool(), 0)
    return (kl_gt_div * word_exist_mat.unsqueeze(-1)).sum() / ((word_exist_mat*know_mask[:,:,:,0]).sum()+1e-30)
    
def word_exist(seq_mask):            
    batch_size, words_num = seq_mask.size(0), seq_mask.size(-1)
    word_exist_matrix = (1-seq_mask).repeat(1, words_num).reshape(-1, words_num, words_num)
    diag_mask = (1-torch.eye(words_num).unsqueeze(0).repeat(batch_size, 1,1))
    if seq_mask.is_cuda:
        diag_mask = diag_mask.cuda()            
    word_exist_matrix = word_exist_matrix * torch.transpose(word_exist_matrix,1,2) 
    return word_exist_matrix
...
def get_new_mask(encoder_outputs,encoder_mask,max_k=max_k,reverse=reverse):
    encoder_outputs_test = copy.copy(encoder_outputs)
    encoder_outputs_test = encoder_outputs_test.transpose(1,0).transpose(-2, -1)
    d_k = encoder_outputs_test.size(-1)
    scores = torch.matmul(encoder_outputs_test, encoder_outputs_test.transpose(-2, -1)) \
             / math.sqrt(d_k)
    p_attn = F.softmax(scores, dim = -2)
    p_attn_1 = p_attn.sum(dim=-1)
    v,index = p_attn_1.topk(max_k,largest=reverse)

    mask = torch.ones(encoder_outputs.transpose(1,0).shape)
    #print(mask.shape)
    #print(mask.sum())
    #mask[:,:,index.squeeze()] = 0
    for i in range(index.size(0)):
        mask[i,:,index[i,:].squeeze()] = 0
    #mask.index_fill_(2, index, 0)
    #print(mask.sum())
    #print(mask.shape)

    #print(encoder_outputs_1_mask.shape)
    #print(encoder_outputs_1_mask.sum())
    #print('encoder_mask.shape')
    #print(encoder_mask.shape)
    #print('mask.shape')
    #print(mask.shape)
    
    new_mask = encoder_mask + mask.cuda().transpose(1,0)
    #print(new_mask.sum())
    new_mask_1 = new_mask >= 1
    new_mask_1 = new_mask_1.float()
    #print(new_mask_1.sum())
    return new_mask_1
...
def train_tree(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               ...
               merge_optimizer, output_lang, num_pos, batch_graph, idx_batch, sf_dic, predict_1, generate_1, merge_1, predict_1_optimizer, generate_1_optimizer, merge_1_optimizer, encoder_mask, alpha, temp=0.5, log_prior=None, log_gt_prior=None, 
               eval_know_inputs=None, know_gt_ww=None, common_dict=None, prior_lam=0.1, gt_prior_lam=0.01, english=False):
    ...
    predict_1_optimizer.zero_grad()
    generate_1_optimizer.zero_grad()
    merge_1_optimizer.zero_grad()
    # Run words through encoder
    
    word_exist_mat = word_exist(seq_mask)
    encoder_outputs, problem_output, word_word, ww_prob, word_op, wo_prob = encoder(input_var, input_length, batch_graph, word_exist_mat, predict.ops, temp=temp, hard=False)
    
    train_index = random.sample(range(len(eval_know_inputs)), int(0.5*len(eval_know_inputs)))
    ww_prob_train, wo_prob_prior = encoder.encode_prior(eval_know_inputs[train_index].unsqueeze(1), predict.ops)
    know_gt_ww_train = know_gt_ww[train_index][:,train_index]
    
    if log_prior is not None:
        # prior kl divergence
        loss_kl = kl_categorical(ww_prob_train, log_prior, know_gt_ww_train.unsqueeze(0).unsqueeze(-1))
        
        wo_know_mask = torch.zeros((1, know_gt_ww_train.size(0), predict.ops.size(0), 1))
        if USE_CUDA:
    ...
    if log_gt_prior is not None:
        # prior kl divergence
        loss_gt_kl = kl_categorical(ww_prob_train, log_gt_prior, (1-know_gt_ww_train).unsqueeze(0).unsqueeze(-1))
    else:
        loss_gt_kl = 0
    ...
    # -------------------[#2 Decoder Stage]--------------------------------
    # Prepare input and output variables
    encoder_outputs_1_mask = encoder_mask[:encoder_outputs.size(1),:max(input_length),:].transpose(1,0).float().cuda()
    #encoder_outputs_1_mask = get_new_mask(encoder_outputs,encoder_outputs_1_mask).float()
    #print(encoder_outputs_1_mask.shape)
    #print(encoder_outputs.shape)
    encoder_outputs_1 = encoder_outputs*encoder_outputs_1_mask.float()
    
    #problem_output_1_mask = torch.cuda.FloatTensor(problem_output.shape).uniform_() < 0.95
    #problem_output_1 = problem_output*problem_output_1_mask.float()
    ...
    for t in range(max_target_length):
        num_score, op, current_embeddings, current_context, current_nums_embeddings = predict_1(
            node_stacks_1, left_childs_1, encoder_outputs_1, all_nums_encoder_outputs_1, padding_hidden, word_word, word_op, word_exist_mat, seq_mask, num_mask)
    ...
    batch_sf = get_softtarget_vector(idx_batch,sf_dic)
    test_batch_sf = torch.stack(batch_sf,dim=0)
    test_tensor = pad_tensor(test_batch_sf, all_node_outputs.size(1), all_node_outputs.size(2))
    ...
    if USE_CUDA:
        ...
        sf_target = test_tensor.float().cuda()
    ...
    #print(loss2.shape)
    #loss = loss_0 + loss_1
    loss = (1-alpha)*loss1 + alpha*loss2 + (1-alpha)*loss3 + alpha*loss4 + 0.1*cos_loss + prior_lam*(loss_kl +loss_wo_kl) + gt_prior_lam*loss_gt_kl
    loss.backward()
    # clip the grad
    ...
    predict_1_optimizer.step()
    generate_1_optimizer.step()
    merge_1_optimizer.step()
    return loss.item() # , loss_0.item(), loss_1.item()

def evaluate_tree_valid(input_batch, input_length, generate_nums, encoder, predict, generate, merge, output_lang, num_pos, batch_graph, predict_1, generate_1, merge_1, gate, gate_optimizer, test_batch_2, test_batch_4, test_batch_6, beam_size=5, english=False, max_length=MAX_OUTPUT_LENGTH):
    ...
    #print(encoder_outputs.shape)
    #print(encoder_outputs)
    #current_context = current_context.cuda()
    #encoder_outputs = encoder_outputs.cuda()
    gate_0 = gate(encoder_outputs,current_context)
    # ---------------------[#2 Decoder]--------------------------------------
    # Prepare input and output variables
    node_stacks_1 = [[TreeNode(_)] for _ in problem_output.split(1, dim=0)]

    num_size_1 = len(num_pos)
    ...
    #current_context = current_context.cuda()
    #encoder_outputs = encoder_outputs.cuda()
    gate_1 = gate(encoder_outputs,current_context)
    
    res_0 = beams[0].out
    res_1 = beams[1].out
    val_ac, equ_ac, _, _ = compute_prefix_tree_result(res_0, test_batch_2, output_lang, test_batch_4, test_batch_6)
    val_ac_1, equ_ac_1, _, _ = compute_prefix_tree_result(res_1, test_batch_2, output_lang, test_batch_4, test_batch_6)
    label = torch.from_numpy(np.array([1 if val_ac else 0,1 if val_ac_1 else 0])).view(-1,2).float()
    logit = torch.cat((gate_0,gate_1),dim=1)
    if USE_CUDA:
        label = label.cuda()
        logit = logit.cuda()
    loss = soft_cross_entropy_loss(logit,label)
    loss.backward()
    gate_optimizer.step()
    return loss.item()

def evaluate_tree(input_batch, input_length, generate_nums, encoder, predict, generate, merge, output_lang, num_pos, batch_graph, predict_1, generate_1, merge_1, temp=0.5, thre=0.5, beam_size=5, english=False, max_length=MAX_OUTPUT_LENGTH):
    ...
    word_exist_mat = word_exist(seq_mask)
    encoder_outputs, problem_output, word_word, _, word_op, _ = encoder(input_var, [input_length], batch_graph, word_exist_mat, predict.ops, temp=temp, thre=thre, hard=True)
    ...
    for t in range(max_length):
        ...
        while len(beams) > 0:
            ...
            num_score, op, current_embeddings, current_context, current_nums_embeddings = predict(
                b.node_stack, left_childs, encoder_outputs, all_nums_encoder_outputs, padding_hidden,
                word_word, word_op, word_exist_mat, seq_mask, num_mask)
    ...
    for t in range(max_length):
        ...
        while len(beams_1) > 0:
            ...
            num_score, op, current_embeddings, current_context, current_nums_embeddings = predict_1(
                b.node_stack, left_childs_1, encoder_outputs, all_nums_encoder_outputs_1, padding_hidden, word_word, word_op, word_exist_mat, seq_mask, num_mask)
...
def evaluate_know(encoder, inputs, gt_ww, know_gt_ww, temp, thre):
    encoder.eval()
    ...
    word_word = word_word_prob > thre
    accuracy = torch.sum((word_word == gt_ww).masked_fill_((1-off_diag).bool(),0)) / torch.sum(off_diag)
    precision = torch.sum(word_word * gt_ww * off_diag) / torch.sum(word_word.masked_fill_((1-off_diag).bool(),0))
    recall = torch.sum(word_word * gt_ww * off_diag) / torch.sum(gt_ww * off_diag)
    logging.info(f"know_acc: {accuracy}, know_pre: {precision}, know_recall: {recall}")
    return word_word_prob

def batch_evaluate_know(encoder, inputs, gt_ww, know_gt_ww, temp, thre):
    encoder.eval()
    word_num = inputs.size(0)
    ...
    for i in range(num_batch):
        batch_input = inputs[i*batch: min((i+1)*batch, word_num)]
        batch_off_diag = off_diag[i*batch: min((i+1)*batch, word_num)]
        batch_gt_ww = gt_ww[i*batch: min((i+1)*batch, word_num)]
        
        batch_embed = encoder.embedding(batch_input) # (batch, dim)
        all_embed = encoder.embedding(inputs) # (word_num, dim)
        
        word_word_prob = encoder.ww_encoder.batch_embed_know(batch_embed, all_embed, temp, hard=True) 
        
        word_word_prob_all.append(word_word_prob)
    word_word_prob = torch.cat(word_word_prob_all, dim=0)
    ...
    return torch.cat(word_word_prob_all, dim=0)
