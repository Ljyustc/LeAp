...
class EncoderSeq(nn.Module):
    def __init__(self, input_size, embedding_size, hidden_size, n_layers=2, dropout=0.5):
        ...
        self.wo_encoder = KnowEncoder1(embedding_size + hidden_size*2, hidden_size, 2)
    ...
    def forward(self, input_seqs, input_lengths, batch_graph, word_exist_mat, ops_embed, temp=0.5, thre=0.5, hidden=None, hard=False):
        # Note: we run this all at once (over multiple batches of multiple sequences)
        embedded = self.embedding(input_seqs)  # S x B x E
        ...
        return pade_outputs, problem_output, word_word, ww_prob, word_op, wo_prob 
...
class Attn_new(nn.Module):
    def __init__(self, hidden_size,nodeout_size):
        super(Attn_new, self).__init__()
        self.hidden_size = hidden_size
        self.nodeout_size = nodeout_size
        self.attn = nn.Linear(hidden_size + nodeout_size, hidden_size)
        self.score = nn.Linear(hidden_size, 1, bias=False)
        self.softmax = nn.Softmax(dim=1)
    ...
    def forward(self, hidden, encoder_outputs, seq_mask=None):
        ...
        # For each position of encoder outputs
        this_batch_size = encoder_outputs.size(1)
        energy_in = torch.cat((hidden, encoder_outputs), 2).view(-1, self.hidden_size + self.nodeout_size)
        attn_energies = self.score(torch.tanh(self.attn(energy_in)))  # (S x B) x 1
        attn_energies = attn_energies.squeeze(1)
        attn_energies = attn_energies.view(max_len, this_batch_size).transpose(0, 1)  # B x S
        if seq_mask is not None:
...
class Gate(nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        self.hidden_size = hidden_size
        #self.nodeout_size = nodeout_size
        self.attn1 = Attn_new(hidden_size,hidden_size)
        #self.attn2 = Attn_new(hidden_size,nodeout_size)
        self.score = nn.Linear(hidden_size, 1, bias=False)

    def forward(self, encoder_outputs, current_context, num_mask=None):
        encoder_attn = self.attn1(current_context.transpose(0, 1), encoder_outputs)
        encoder_context = encoder_attn.bmm(encoder_outputs.transpose(0, 1))  # B x 1 x N
        
        #all_node_outputs = all_node_outputs.transpose(1,0)
        #node_attn = self.attn2(current_context.transpose(0, 1), all_node_outputs)
        #node_context = node_attn.bmm(all_node_outputs.transpose(0, 1))  # B x 1 x N
        #print('node_context.shape')
        #print(node_context.shape)
        
        #gate_in = torch.cat((encoder_context.squeeze(), node_context.squeeze()), 1).view(-1, self.hidden_size+self.nodeout_size)
        gate_in = encoder_context.squeeze().view(-1, self.hidden_size)
        this_batch_size = encoder_context.size(0)
        gate_in = gate_in.view(this_batch_size, -1)
        score = torch.relu(self.score(gate_in))
        return score    
...
class Know_Net(nn.Module):
    ...
    def forward(self, encoder_outputs, ops, s, current_attn, word_word, word_op, word_exist_mat, seq_mask):                    
        # encoder_outputs: B x seq x N, ops: op_num x 2N
        # s: B x 1 x N
