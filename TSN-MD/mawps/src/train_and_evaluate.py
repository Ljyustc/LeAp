...
def train_tree(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               ...
               merge_optimizer, output_lang, num_pos, batch_graph, idx_batch, sf_dic, predict_1, generate_1, merge_1, predict_1_optimizer, generate_1_optimizer, merge_1_optimizer, temp=0.5, log_prior=None, log_gt_prior=None, 
    ...
    ww_prob_prior, wo_prob_prior = encoder.encode_prior(eval_know_inputs.unsqueeze(1), predict.ops)

    if log_prior is not None:
        # prior kl divergence
        loss_kl = kl_categorical(ww_prob_prior, log_prior, know_gt_ww.unsqueeze(0).unsqueeze(-1))
        
        wo_know_mask = torch.zeros((1, know_gt_ww.size(0), predict.ops.size(0), 1))
        if USE_CUDA:
    ...
    if log_gt_prior is not None:
        # prior kl divergence
        loss_gt_kl = kl_categorical(ww_prob_prior, log_gt_prior, (1-know_gt_ww).unsqueeze(0).unsqueeze(-1))
    else:
        loss_gt_kl = 0
    ...
    for t in range(max_target_length):
        num_score, op, current_embeddings, current_context, current_nums_embeddings = predict_1(
            node_stacks_1, left_childs_1, encoder_outputs, all_nums_encoder_outputs_1, padding_hidden, word_word, word_op, word_exist_mat, seq_mask, num_mask)
    ...
    #print(loss2.shape)
    #loss = loss_0 + loss_1
    loss = 0.85*loss1 + 0.15*loss2 + 0.85*loss3 + 0.15*loss4 + 0.1*cos_loss + prior_lam*(loss_kl +loss_wo_kl) + gt_prior_lam*loss_gt_kl
    loss.backward()
    # clip the grad
...
def evaluate_tree(input_batch, input_length, generate_nums, encoder, predict, generate, merge, output_lang, num_pos, batch_graph, predict_1, generate_1, merge_1, temp=0.5, thre=0.5, beam_size=5, english=False, max_length=MAX_OUTPUT_LENGTH):
    ...
    for t in range(max_length):
        ...
        while len(beams_1) > 0:
            ...
            num_score, op, current_embeddings, current_context, current_nums_embeddings = predict_1(
                b.node_stack, left_childs_1, encoder_outputs, all_nums_encoder_outputs_1, padding_hidden, word_word, word_op, word_exist_mat,
                seq_mask, num_mask)
...
def evaluate_know(encoder, inputs, gt_ww, know_gt_ww, temp):
    ...
    logging.info(f"pre@50: {pre}, recall@50: {rec}")
    return word_word_prob

def batch_evaluate_know(encoder, inputs, gt_ww, know_gt_ww, temp):
    encoder.eval()
    word_num = inputs.size(0)
