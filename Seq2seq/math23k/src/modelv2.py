...
def batch_evaluate_know(encoder, inputs, gt_ww, know_gt_ww, temp, device=None):
	encoder.eval()
	word_num = inputs.size(0)
	embedded = inputs  # (word_num, dim)

	# off-diagonal, not consider self-loop
	off_diag = torch.FloatTensor(np.ones([word_num, word_num]) - np.eye(word_num))
	if device:
		off_diag = off_diag.to(device)
	...
	for i in range(num_batch):
		...
		batch_embed = embedded[i*batch: min((i+1)*batch, word_num)] # (batch, dim)
		all_embed = embedded # (word_num, dim)
	...
	pre, rec = eval_know_k(word_num, word_word_prob*(1-know_gt_ww), gt_ww*(1-know_gt_ww), 50, device)
	print("know_pre: ",pre, "know_recall: ",rec)
	return torch.cat(word_word_prob_all, dim=0)

def eval_know_k(common_word_count, ww, gt_ww, k, device=None):
...
class Seq2SeqModel(nn.Module):
	...
	def trainer(self, ques, input_seq1, input_seq2, input_len1, input_len2, config, device=None ,logger=None,
				temp=0.5, log_prior=None, log_gt_prior=None, eval_know_inputs=None, know_gt_ww=None, common_dict=None, 
				prior_lam=0.1, gt_prior_lam=0.1):
		'''
			Args:
		...
		for i in sorted_len:
			sort_seq_mask.append([0 for _ in range(i)] + [1 for _ in range(i, max_len)])
		sort_seq_mask = torch.ByteTensor(sort_seq_mask)
		sort_word_exist_mat = word_exist(sort_seq_mask)

		if device:
			sort_word_exist_mat = sort_word_exist_mat.to(device)
			sort_seq_mask = sort_seq_mask.to(device)

		word_exist_mat = sort_word_exist_mat.index_select(0, orig_idx)
		seq_mask = sort_seq_mask.index_select(0, orig_idx)

		encoder_outputs, encoder_hidden, word_word, ww_prob, word_op, wo_prob = self.encoder(sorted_seqs, sorted_len, orig_idx, self.device, self.decoder.ops, sort_word_exist_mat=sort_word_exist_mat, temp=temp, hard=False)
		
		train_index = random.sample(range(len(eval_know_inputs)), int(0.3*len(eval_know_inputs)))
		ww_prob_train, wo_prob_prior = self.encoder.encode_prior(self.embedding1(eval_know_inputs[train_index].unsqueeze(1)), self.decoder.ops)
		...
		if log_prior is not None:
			...
			wo_know_mask = torch.zeros((1, know_gt_ww_train.size(0), self.decoder.ops.size(0), 1))
			if device:
				wo_know_mask = wo_know_mask.to(device)
		...
		self.loss = prior_lam*(loss_kl + loss_wo_kl) + gt_prior_lam*loss_gt_kl
		...
		if use_teacher_forcing:
			for step in range(target_len):
				if self.config.use_attn:
					decoder_output, decoder_hidden, decoder_attention, _ = self.decoder(decoder_input, decoder_hidden, encoder_outputs, word_word, word_op, word_exist_mat, seq_mask)
				else:
	...
	def greedy_decode(self, ques, input_seq1=None, input_seq2=None, input_len1=None, input_len2=None, validation=False, return_probs = False,
		...
		with torch.no_grad():
			...
			if self.device:
				sort_word_exist_mat = sort_word_exist_mat.to(self.device)
				sort_seq_mask = sort_seq_mask.to(self.device)

			word_exist_mat = sort_word_exist_mat.index_select(0, orig_idx)
			seq_mask = sort_seq_mask.index_select(0, orig_idx)

			encoder_outputs, encoder_hidden, word_word, ww_prob, word_op, wo_prob = self.encoder(sorted_seqs, sorted_len, orig_idx, self.device, self.decoder.ops, sort_word_exist_mat=sort_word_exist_mat, temp=temp, thre=thre, hard=True)
...
def train_model(model, fold, train_dataloader, val_dataloader, voc1, voc2, device, config, logger, epoch_offset= 0, min_val_loss=float('inf'), max_val_bleu=0.0, max_val_acc = 0.0, min_train_loss=float('inf'), max_train_acc = 0.0, best_epoch = 0, writer= None,
				log_prior=None, log_gt_prior=None, eval_know_inputs=None, know_gt_ww=None, gt_ww=None, common_dict=None):
	'''
		Add Docstring
	'''
	...
	for epoch in range(1, config.epochs + 1):
		temp = config.temp - (config.temp - 0.1) / (config.epochs-1) * epoch
		prior_lam = 0.1
		gt_prior_lam = 0.1
		od = OrderedDict()
		...
		for data in train_dataloader:
			...
			loss = model.trainer(ques, sent1_var, sent2_var, input_len1, input_len2, config, device, logger, temp=temp, 
					log_prior=log_prior, log_gt_prior=log_gt_prior, eval_know_inputs=eval_know_inputs, know_gt_ww=know_gt_ww, 
					common_dict=common_dict, prior_lam=prior_lam, gt_prior_lam=gt_prior_lam)
			train_loss_epoch += loss

			if config.show_train_acc:
				model.eval()

				_, decoder_output, _ = model.greedy_decode(ques, sent1_var, sent2_var, input_len1, input_len2, validation=True, temp=temp, thre=config.thre)
		...
		# if (epoch+1) % 10 == 0 or epoch ==1:
			# word_word = batch_evaluate_know(model.encoder, model.embedding1(eval_know_inputs), gt_ww, know_gt_ww, config.temp, model.device)
		# if epoch == config.epochs-1:
			# torch.save(eval_know_inputs, str(fold)+"_eval_know_inputs.pt")
			# torch.save(word_word, str(fold)+"_word_word.pt")
			# np.save(str(fold)+"_word2index.npy", voc1.w2id)

			# word_op = model.encoder.wo_encoder.embed_know(model.embedding1(eval_know_inputs), model.decoder.ops, config.temp, hard=True) 
			# torch.save(word_op, str(fold)+"_word_op.pt")
