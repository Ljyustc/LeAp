			...
			for epoch in range(config.epochs):
				...
				for idx in range(len(input_lengths)):
					...
					loss = train_tree(
						...
						num_pos_batches[idx], temp=temp, log_prior=log_prior, log_gt_prior=log_gt_prior, eval_know_inputs=eval_know_inputs, know_gt_ww=know_gt_ww, common_dict=common_dict, prior_lam=prior_lam, gt_prior_lam=gt_prior_lam)
					loss_total += loss
					...
					with torch.no_grad():
						for train_batch in train_pairs:
							...
							train_res = evaluate_tree(config, train_batch[0], train_batch[1], generate_num_ids, embedding, encoder, predict, generate,
													 merge, input_lang, output_lang, train_batch[5], temp=config.temp, thre=config.thre, beam_size=config.beam_size)
				...
				for test_batch in test_pairs:
					...
					test_res = evaluate_tree(config, test_batch[0], test_batch[1], generate_num_ids, embedding, encoder, predict, generate,
											 merge, input_lang, output_lang, test_batch[5], temp=config.temp, thre=config.thre, beam_size=config.beam_size)
