...
def train_tree(config, input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
			   ...
			   merge_optimizer, input_lang, output_lang, num_pos, temp, log_prior, log_gt_prior, eval_know_inputs, know_gt_ww, common_dict, prior_lam=0.1, gt_prior_lam=0.1, english=False):
	...
	encoder_outputs, problem_output, word_word, ww_prob,  word_op, wo_prob = encoder(embedded, input_length, orig_idx, word_exist_mat, predict.ops, temp=temp, hard=False)
...
def evaluate_tree(config, input_batch, input_length, generate_nums, embedding, encoder, predict, generate, merge, input_lang, output_lang, num_pos, temp=0.5, thre=0.5,
				  beam_size=5, english=False, max_length=MAX_OUTPUT_LENGTH):
	...
	word_exist_mat = word_exist(seq_mask)
	encoder_outputs, problem_output, word_word, ww_prob, word_op, wo_prob = encoder(embedded, [input_length], orig_idx, word_exist_mat, predict.ops, temp=temp, thre=thre, hard=True)
